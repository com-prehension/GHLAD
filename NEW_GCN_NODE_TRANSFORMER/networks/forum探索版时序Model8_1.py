...
class TransGRUNet_1_4(torch.nn.Module):
    ...
    def process_graph_with_transformer(self, cluster_assignments, call_sequences, x, batch):
        ...
        call_sequences_cpu = call_sequences.cpu()
        x_cpu = x.cpu()

        # 初始化全局结果张量，与输入 x 形状相同，且在 CPU 上
        global_outputs = torch.zeros_like(x_cpu)

        batch_size = batch_cpu.max().item() + 1

        # 缓存每个批次的节点索引
        batch_nodes = [None] * batch_size
        for i in range(batch_size):
            batch_nodes[i] = (batch_cpu == i).nonzero(as_tuple=True)[0]
        ...
        for i in range(batch_size):
            ...
            graph_clusters = cluster_assignments_cpu[nodes_in_batch]
        ...
        for i in range(batch_size):
            nodes_in_batch = batch_nodes[i]
            graph_features = x_cpu[nodes_in_batch]
            graph_calls = call_sequences_cpu[nodes_in_batch]
            graph_clusters = cluster_assignments_cpu[nodes_in_batch]
            ...
            for layer_id in range(graph_clusters.max().item()+1):
                ...
                if pad_size > 0:
                    padding = torch.zeros((pad_size, sorted_features.size(1)))
                    sorted_features = torch.cat([sorted_features, padding], dim=0)
                    padding_indices = -torch.ones(pad_size, dtype=torch.long)
                ...
                layer_masks_list.append(torch.tensor([0] * seq_len + [1] * pad_size, dtype=torch.bool))
        ...
        # 将数据移动到 GPU
        sequential_data = sequential_data.to(x.device)
        mask = mask.to(x.device)
        indices = indices.to(x.device)
        global_outputs = global_outputs.to(x.device)
